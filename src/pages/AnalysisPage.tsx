...
export const ProgramPage: React.FC = () => {
  ...
  // ✅ CLÉ MAGIQUE POUR FORCER LE RESET
  ...
  // ✅ UPLOAD AVEC RESET MEMOIRE
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    const files = event.target.files;
    if (!files || files.length === 0) return;

    // 1. On vide tout visuellement
    setLoading(true);
    setReport(null); 
    
    // Petit délai pour laisser l'interface respirer
    await new Promise(r => setTimeout(r, 100));

    try {
      const file = files[0];
      
      // 2. Analyse avec le moteur
      const newReport = await ImageEngine.analyzeScreenshot(file, null);
      
      // 3. Mise à jour des données
      setReport(newReport);
      
      // 4. INCROYABLEMENT IMPORTANT : On change la clé pour recréer le tableau
  ...
  const handleSaveToAnalysis = () => {
    ...
    try {
      ...
      // On peut vider après sauvegarde si tu veux
      // setReport(null); 
            ...
            <div className="flex flex-col items-center justify-center h-full">
              {loading ? (
                <div className="text-center animate-pulse">
                    ...
                    <p className="text-white font-bold">Analyse de l'image...</p>
                </div>
              ) : (
                <>
